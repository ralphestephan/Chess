...
function drop(ev) {
  ...
  if (
    ...
    changeTurn(); // Call changeTurn() instead of toggling isWhiteTurn directly
  ...
  if (
    ...
    destinationSquare.appendChild(piece);
    changeTurn(); // Ensure changeTurn() is called here as well
    updateBoardSquaresArray(
      startingSquareId,
      destinationSquareId,
      boardSquaresArray
...
function updateTurnIndicator() {
  ...
  turnIndicator.classList.remove('centered');

  // Trigger reflow to restart the animation
  void turnIndicator.offsetWidth;

  // Add the 'centered' class to position and show the indicator
  turnIndicator.classList.add('centered');

  // After a delay, remove the 'centered' class to move it to its final position and fade out
  setTimeout(function() {
      turnIndicator.classList.remove('centered');
...
function changeTurn() {
  ...
  updateTurnIndicator();
  startTimer(); // Restart the timer on turn change
}
...
function handleGameOver(winner) {
  ...
  // For example, disable piece movement
  disableBoard();
}

function disableBoard() {
  for (let i = 0; i < boardSquares.length; i++) {
    boardSquares[i].removeEventListener("dragover", allowDrop);
    boardSquares[i].removeEventListener("drop", drop);
  }
  for (let i = 0; i < pieces.length; i++) {
...
// Initialize timers and start the game timer
